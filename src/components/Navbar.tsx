      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex justify-between items-center">
          <div className="flex items-center">
            <span className={`text-xl md:text-2xl font-bold ${isScrolled ? 'text-[#9D2235]' : 'text-white'}`}>
              Bee's Style
            </span>
          </div>
          ...
          <div className="hidden md:flex space-x-8">
            {['home', 'about', 'gallery', 'menu', 'testimonials', 'location'].map((item) => (
              <button
                key={item}
                onClick={() => scrollToSection(item)}
                className={`${
                  isScrolled ? 'text-gray-800 hover:text-[#9D2235]' : 'text-white hover:text-gray-200'
